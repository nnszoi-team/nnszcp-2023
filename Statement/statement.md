...
小 P 说：「化学反应的活化能是整个化学反应进程中的能量的**最大值**，即 $\max_{i=0}^{n} \{a_i\}$。」

小 Q 说：「化学反应的活化能是某个反应步骤进行前后，总能量**变化量**的**最大值**。即 $\max_{i=0}^{n-1} \{a_{i + 1} - a_{i}\}$。」
...
如图，在平面直角坐标系中，抛物线 $ y = x\left(k - x\right) $ 与直线 $ y = -1 $ 相交。抛物线与 $ x $ 轴的另一个交点为 $\mathrm{A}$。设**线段** $\mathrm{OA}$ 上存在一动点 $\mathrm{P}$，过点 $\mathrm{P}$ 作 $y$ 轴的平行线交抛物线于点 $\mathrm{B}$，交直线 $y = -1$ 于点 $\mathrm{C}$。试求 $\mathrm{OB}^2 + \mathrm{AC}^2$ 的最大值。
...
接下来的 $ T $ 行，每行包含两个正整数 $ a, b $，表示 $ k = \frac{a}{b} $。
...
给定两个正整数 $n, k$。你需要**构造**一个 $n\times n$ 的矩阵 $A$，其中 $A_{i, j}$ 是**不重复**的整数且 $A_{i, j} \in \left[ 0, k \right]$，使得其满足：
...
定义一个长度为 $n$ 的序列 $t$ 是**严格不下降**序列，当且仅当对所有 $1 \le i \le j \le n$，都满足 $t_i \le t_j$，也就是说这个序列中不存在逆序对。
...
小 K 可以先购买 $3$ 号购车券，此时对于 $1$ 号购车券，他只剩一张关联的购车券还未拥有（$2$ 号购车券），因此他能够购买 $1$ 号购车券。总共的购买顺序为 $\left(3, 1, 2\right)$。类似的，他也能以 $\left(2, 1, 3\right)$，$\left(2, 3, 1\right)$，$\left(3, 2, 1\right)$ 的顺序购买，一共 $4$ 种购买方式。
...
<img src=".\assets\flower.jpg" alt="flower" style="zoom: 33%;" />

小 S 是一个可爱的女孩子，她一直梦想着拥有一座美丽的花园，喜欢五颜六色的花。

这天她买了一本关于花卉培养的书，按照书中的指引，她又买了 $n$ 个花盆，从左到右依次标号 $1, 2, \dots, n$，准备在这些花盆中种下一些花；具体地，对于第 $i$ 种颜色的花盆，她可以种下不超过 $10^9$ 株花，当然也可以不种花。
...
第一行两个整数 $n, q$，分别代表小 S 购买的花盆数量和书里美化花卉的秘诀数量。

接下来 $q$ 行，每行三个整数 $l_i, r_i, v_i$，代表每一条美化花卉的秘诀。
...
第 $1$ 条秘诀 $\left(1, 3, 2\right)$ 即 $\left(1001011 \times 34213 \times 6232\right) \bmod 3 = 2$。

第 $2$ 条秘诀 $\left(1, 2, 1\right)$ 即 $\left(1001011 \times 34213\right) \bmod 3 = 1$。

第 $3$ 条秘诀 $\left(2, 3, 2\right)$ 即 $\left(34213 \times 6232\right) \bmod 3 = 2$。

第 $4$ 条秘诀 $\left(3, 5, 0\right)$ 即 $\left(6232 \times 0 \times 21\right) \bmod 3 = 0$。
...
0. （2 分）$q = 0$；
1. （23 分）$1 \le n, q \le 10$；
2. （27 分）$1 \le n, q \le 2 \times 10^3$；
3. （16 分）对所有 $1 \le j \le q$，$l_j = r_j$；
4. （32 分）无额外限制。
...
形式化地，网格的四个顶点分别是 $\left(0, 0 \right)$、$\left(0, p\right)$、$\left(p, p\right)$、$\left(p, 0\right)$，边界为折线 $\left(0, 0\right) \rightsquigarrow \left(0, p\right) \rightsquigarrow \left(p, p\right) \rightsquigarrow \left(p, 0\right) \rightsquigarrow \left(0, 0\right)$。
