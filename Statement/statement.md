...
* $0 \le v_i < 4$；
* 存在一种序列 $a_j$，能够满足所有的 $\left(l_i, r_i, v_i\right)$；
* 所有输入数据均为整数。
...
小 E 想让这片天空看上去不那么单调，所以他选择了一个点 $\left( \dfrac{a}{b}, \dfrac{c}{d} \right)$（这个点的横纵坐标不大于 $1$），想在上面释放一颗星星。为了尽快达成目标，他想得到一个发动超能力的方案，使得在他发动超能力**至多 $1500$ 次**以后，点 $\left( \dfrac{a}{b}, \dfrac{c}{d} \right)$ 上面有一颗星星。
