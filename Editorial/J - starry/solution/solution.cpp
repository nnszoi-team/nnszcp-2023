...
int main() {
	...
	auto parrallel_y_grid_binary_line = [&](int x, int bound, int L, int R) {
		...
		draw_op1(L, 2), draw_op1(R, 2), draw_op1(L, 0), draw_op1(R, 0),
			draw_op1(L, 1), draw_op1(R, 1);
	...
	auto parrallel_x_grid_binary_line = [&](int y, int bound, int L, int R) {
		...
		draw_op1(2, L), draw_op1(2, R), draw_op1(0, L), draw_op1(0, R),
			draw_op1(1, L), draw_op1(1, R);
		for (int i = 0; i < bound; ++i) {
			F mid = (l + r) * F(1, 2);
			auto sm = draw_op2(list[ul], list[mr], list[ur], list[ml]);
			auto tm = draw_op2(list[dl], list[mr], list[dr], list[ml]);
	...
	auto shift_parrallel_x_line = [&](P ul, P um, P ur,
									  i64 cur) -> std::pair<P, P> {
		...
		for (int i = b - 2; i >= 0; --i) {
			...
			auto new_um =
				draw_op2(list[dm], list[Um], list[new_ul], list[new_ur]);
			auto new_sl = draw_op2(list[dm], list[stmpl], list[dl], list[Ul]);
			auto new_sr = draw_op2(list[dm], list[stmpr], list[dr], list[Ur]);
			auto new_sm =
				draw_op2(list[dm], list[Um], list[new_sl], list[new_sr]);

			auto next_l = draw_op2(list[new_sl], list[new_um], list[new_sm],
								   list[new_ul]);
			auto next_r = draw_op2(list[new_sr], list[new_um], list[new_sm],
								   list[new_ur]);

			new_sl = draw_op2(list[new_sl], list[new_ul], list[next_l],
							  list[next_r]);
			new_sm = draw_op2(list[new_sm], list[new_um], list[next_l],
							  list[next_r]);
			new_sr = draw_op2(list[new_sr], list[new_ur], list[next_l],
							  list[next_r]);
			...
			if ((cur >> i) & 1) {
				...
				new_um =
					draw_op2(list[um], list[sm], list[new_ul], list[new_ur]);
				sl = ul, sm = um, sr = ur;
				ul = new_ul, um = new_um, ur = new_ur;
			}
		}
	...
	auto shift_parrallel_y_line = [&](P ul, P um, P ur,
									  i64 cur) -> std::pair<P, P> {
	...
	auto [A, B] = [&parrallel_x_grid_binary_line, &parrallel_y_grid_binary_line,
				   &bound, &draw_op1, &draw_op2, &list,
				   &shift_parrallel_x_line](i64 a, i64 b) {
	...
	auto [C, D] = [&parrallel_x_grid_binary_line, &parrallel_y_grid_binary_line,
				   &bound, &draw_op1, &draw_op2, &list,
				   &shift_parrallel_y_line](i64 a, i64 b) {
