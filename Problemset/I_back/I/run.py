...
def generate(n, q, p, t, only, sc, subtask, count, title, num):
    ...
    system(
        f"gen --n={n} --q={q} --p={p} --t={t} --only={only} > ./data/{f'{count:02d}'}_{subtask}_{title}_{f'{num:02d}'}.in"
    )
...
for i in range(1, 3):
    generate(
        20000 - i + 1, 20000 - i + 1, 5, 150, 0, 41, 4, count, "square_root_range", i
    )
    count += 1
...
for i in range(1, 3):
    generate(
        100000 - i + 1, 100000 - i + 1, 10, 90000, 0, 41, 4, count, "big_range_huge", i
    )
    count += 1
